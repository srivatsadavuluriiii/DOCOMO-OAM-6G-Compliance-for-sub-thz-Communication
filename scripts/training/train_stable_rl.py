...
from utils.state_dimension_validator import (
    validate_state_dimensions,
    get_state_dimension_report,
)
import time
...
def train(args: argparse.Namespace) -> None:
    ...
    if args.config.endswith('.yaml'):
        ...
        # Hierarchical config: load RL config, then overlay stable reward config
        rl_cfg = load_hierarchical_config("rl_config_new")
        stable_cfg = load_hierarchical_config(args.stable_config)
        # stable_cfg may only contain stable_reward/reward; merge over rl_cfg
        from utils.config_utils import merge_configs as _merge
        config = _merge(rl_cfg, stable_cfg)
    ...
    # Create agent
    # Support both flat and hierarchical (rl_base) keys
